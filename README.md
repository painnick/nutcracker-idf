...
| 사운드 | DFPlayer Mini (UART TX 전용), L1/R1 볼륨, NVS 저장. 대기음 0001은 미연결일 때만. 기기마다 연결음 0004. 연결 중 30초 무조작이면 60초마다 BGM 0006~0009 |
...
| 연결 성공 | 연결음 0004. 패드 럼블 (400ms). 대기음 0001 중지 |
| 연결 중 30초간 조작 없음 | 60초마다 BGM 0006~0009 중 하나 재생 |
...
TV 쪽이 보드 **앞(top)** 센서입니다. 앞으로 기울이면 전진, 오른쪽으로 기울이면 우측 이동입니다. 우측 스틱 평행이동과 동일하게 w=0 으로 믹스합니다. 작은 체중 이동은 무시하고, 대각선 기울기는 약 15°~75° 구간에서 45°로 맞춥니다. 두 축이 같이 실리면 이동 문턱을 1400g에서 900g으로 낮춥니다.

게임패드와 동시에 연결되면 보드가 움직일 때는 **휠은 밸런스 보드**를 따릅니다. 보드가 정지(무게 없음 또는 이동 문턱 이하)로 판단되면 패드 스틱으로 주행합니다. 포탑, 발사, 레이더, 볼륨 등 버튼은 게임패드가 담당합니다. 보드만 끊기면 다시 패드 스틱으로 주행합니다.
...
- **DFPlayer**: TX 전용 (GPIO 23 → 모듈 RX). 수신 핀 미사용. 대기 중 짧은 효과음은 타이머 후 0001 루프로 복귀한다. 기기가 1대라도 연결되면 대기음은 끄고, 연결될 때마다 0004를 재생한다.
