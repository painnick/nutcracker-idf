...
| 가습기 | GPIO4, X 버튼 시 2초 ON. 네오픽셀 주황 1.6초 유지 후 0.4초 동안 흰색 |
...
모터 PWM과 nSLEEP에는 스트래핑 위험 핀(0, 2, 12)을 쓰지 않습니다.
...
| 포탑 IN1 | 16 | DRV8833 #3 |
| 포탑 IN2 | 17 | DRV8833 #3 |
| 모터 nSLEEP | 23 | DRV8833 #1/#2/#3 공통, HIGH=동작 |
...
- **nSLEEP (GPIO23)**: 세 드라이버 공통. 휠 또는 포탑이 움직이기 직전에 HIGH, 전부 정지하면 LOW. GPIO12(MTDI)는 리셋 시 플래시 전압 스트래핑이라 사용하지 않는다.
