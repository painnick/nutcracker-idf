...
| 연결 중 30초 무조작 | 60초마다 BGM 0006~0009 중 하나 |

### 4.3.1 Wii Balance Board

무게 중심 앞/뒤는 전후, 좌/우는 평행이동. Yaw 없음. 대각선 기울기는 약 15°~75°에서 45°로 맞춘다. 두 축이 함께 실리면 이동 문턱을 1400g에서 900g으로 낮춘다.

게임패드와 동시에 연결되면 보드가 움직일 때는 휠은 밸런스 보드를 쓰고, 보드가 정지면 패드 스틱을 쓴다. 포탑/버튼은 게임패드. Bluepad32 최대 장치 수는 2이다.
...
- 대기음은 `play_loop(0001)`. 기기가 1대라도 연결되면 대기음을 끄고, 연결될 때마다 0004를 재생한다
- 짧은 효과음(0002/0003/0004/0005)은 1회 재생 후, 대기 중일 때만 타이머로 IDLE 루프 복귀
