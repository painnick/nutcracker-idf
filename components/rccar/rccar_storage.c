...
static const char *TAG = "rccar_storage";
static const char *NVS_NAMESPACE = "rccar";
static const char *NVS_KEY_VOLUME = "vol";

static uint8_t s_volume = RCCAR_VOLUME_DEFAULT;
...
esp_err_t rccar_storage_init(void)
{
    ...
    if (ret != ESP_OK) {
        ...
        s_volume = RCCAR_VOLUME_DEFAULT;
        return ret;
    }

    uint8_t v;
    ret = nvs_get_u8(s_handle, NVS_KEY_VOLUME, &v);
    if (ret == ESP_OK && v >= RCCAR_VOLUME_MIN && v <= RCCAR_VOLUME_MAX) {
        s_volume = v;
        ESP_LOGI(TAG, "volume loaded %u", (unsigned)s_volume);
    } else {
        s_volume = RCCAR_VOLUME_DEFAULT;
        ESP_LOGI(TAG, "volume default %u", (unsigned)s_volume);
    }
    return ESP_OK;
}
...
{
    if (vol < RCCAR_VOLUME_MIN) {
        vol = RCCAR_VOLUME_MIN;
    }
    if (vol > RCCAR_VOLUME_MAX) {
        vol = RCCAR_VOLUME_MAX;
    }
    s_volume = vol;
    if (s_handle == 0) {
